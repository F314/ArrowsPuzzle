...
{ 	window.status=" tile " + dd.obj.name.slice(1,3); 
}
...
var rmax=28, posx,posy,kx,ky,bx,by,r,dx,dy     ;

function my_DropFunc()
{ 	window.status=" ";
	posx=dd.obj.x; posy=dd.obj.y; kx=posx; ky=posy;
      if((kx>(560-rmax))*(kx<(784+rmax))*(ky>(280-rmax))*(ky<(504+rmax))){
       bx = 56*(Math.floor(kx/56) + Math.round((kx%56)/56));
       by = 56*(Math.floor(ky/56) + Math.round((ky%56)/56));
       dx = kx-bx; dy = ky-by;
        r = Math.sqrt(dx*dx+dy*dy);
        if(r<rmax){posx-=dx; posy-=dy;}  } 
            dd.obj.moveTo((posx),(posy));
 }
